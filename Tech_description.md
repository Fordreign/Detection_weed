...
# Модель
Основное предложение от YOLO заключается в формулировке обнаружения объектов как проблемы однопроходной (one-stage) регрессии.

"YOLOv8 является последней версией модели обнаружения объектов YOLO. Эта последняя версия имеет ту же архитектуру, что и предыдущие версии, но внесла множество улучшений по сравнению с более ранними версиями YOLO, таких как новая архитектура нейронной сети, использующая как Feature Pyramid Network (FPN), так и Path Aggregation Network (PAN), а также новое инструментарий разметки, упрощающий процесс аннотации. Этот инструментарий разметки содержит несколько полезных функций, таких как автоматическая разметка, ярлыки ярлыков и настраиваемые горячие клавиши.

FPN работает путем постепенного уменьшения пространственного разрешения входного изображения при одновременном увеличении числа каналов признаков. В результате создаются карты признаков, способные обнаруживать объекты различных масштабов и разрешений. Архитектура PAN, с другой стороны, агрегирует признаки с разных уровней сети через пропускные соединения. Таким образом, сеть может лучше улавливать признаки на нескольких масштабах и разрешениях, что критично для точного обнаружения объектов разных размеров и форм."

[цитата](https://arxiv.org/pdf/2305.09972.pdf)

YOLOv8 состоит из трех основных компонентов:

Основа (backbone): Отвечает за извлечение признаков из входного изображения с использованием модифицированной архитектуры CSPDarknet и кросс-стейдж-частичных соединений (Cross Stage Partial connections).

Шейка (neck): Соединяет основу и голову модели, улучшая и агрегируя признаки. Использует модули Spatial Pyramid Pooling (SPP) и CSP-Path Aggregation Network (PAN) для обработки признаков на разных уровнях.

Голова (head): Отвечает за вывод предсказаний объектов. Состоит из одной головы вывода, которая предсказывает координаты ограничивающих рамок, вероятности классов и оценки уверенности. Использует анкерные рамки (anchor boxes) для предсказания координат и размеров ограничивающих рамок.
